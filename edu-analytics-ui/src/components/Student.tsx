...
const Student: React.FunctionComponent<Student> = ({
    ...
    React.useEffect(() => {
        ...
        const taskStatus = tabValue === 'Assigned' ? 'NotStarted' : tabValue === 'In Progress' ? 'Inprogress' : tabValue;
        Get(`https://edu-classroom-analytics.uc.r.appspot.com/eduAnalytics/student/1/analytics`)
            .then((res) => {
    ...
    const displayAllAssigments = () => {
        console.log('Assigsment list', assigmentList);
...
return (
    <>
        ...
        <VideoModal open={modalStatus} videoId={videoId} onClose={onModalClose} />
    </>
)
}
